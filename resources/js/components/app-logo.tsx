...
export default function AppLogo({ className = '', variant = 'sidebar', showText = true, iconSize = 'md', colorScheme = 'default' }: AppLogoProps) {
    const iconSizes = {
        sm: 'size-4',
        md: 'size-5',
        lg: 'size-6',
    ...
    return (
        <div className={cn('flex items-center gap-3', className)}>
            <div className={cn('flex items-center justify-center rounded-md', colors.container, containerSizes[iconSize])}>
                <AppLogoIconNew className={cn(colors.icon, iconSizes[iconSize])} />
            </div>
            {showText && <span className={cn('font-semibold', colors.text, textSizes[iconSize])}>ThinkTest AI</span>}
        </div>
    );
}
